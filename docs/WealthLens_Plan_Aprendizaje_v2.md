...
> **Nota sobre arquitectura:** En cada módulo se explican los patrones de arquitectura aplicados, anclados al contexto real de WealthLens. No como teoría abstracta, sino como decisiones concretas con sus trade-offs. El objetivo es consolidar criterio arquitectónico a través de la experiencia práctica.
...
**Patrón de arquitectura — Monorepo vs Polyrepo:**  
Antes de crear el repositorio, decisión de diseño: ¿un repo para todo el proyecto o repos separados para frontend y backend? Analizaremos los trade-offs de cada enfoque y justificaremos la elección para WealthLens.
...
**Patrón de arquitectura — Component-Based Architecture y Separation of Concerns:**  
Por qué React organiza la UI en componentes. Qué problema resuelve frente a HTML monolítico. Cómo decidir qué merece ser un componente y qué no. Atomic Design como framework mental para organizar componentes (átomos → moléculas → organismos → páginas).
...
**Patrón de arquitectura — Unidirectional Data Flow y State Management:**  
El patrón Flux que inventó Facebook y por qué cambió el frontend para siempre. Por qué los datos fluyen en una sola dirección en React (y qué problemas evita). Cuándo el estado local de un componente es suficiente y cuándo necesitas estado global. Trade-offs entre Context API, Redux y Zustand.
...
**Patrón de arquitectura — Infrastructure as Code y Immutable Deployments:**  
Por qué el pipeline está definido en un archivo YAML en el repositorio (Infrastructure as Code) y no configurado manualmente en una interfaz. El concepto de artefacto inmutable: por qué no se modifica una app en producción, se reemplaza. Twelve-Factor App: los principios que siguen las aplicaciones cloud-native modernas.
...
**Patrón de arquitectura — Layered Architecture, REST constraints y API Gateway:**  
La separación en capas (presentación → lógica de negocio → datos) y por qué es el patrón más usado en aplicaciones empresariales. Los 6 constraints de REST según Roy Fielding y cuáles estamos aplicando en WealthLens. El patrón API Gateway: por qué el frontend no llama directamente a la base de datos. Serverless vs servidor tradicional: trade-offs reales.
...
**Patrón de arquitectura — Delegation Pattern, Token-based Security y Zero Trust:**  
Por qué delegamos la autenticación a un servicio especializado en lugar de implementarla nosotros (y los desastres que evitamos). El modelo Zero Trust: nunca confiar, siempre verificar. Cómo funciona OAuth 2.0 con el patrón Authorization Code Flow. Stateless authentication: por qué los JWT eliminan la necesidad de sesiones en servidor.
...
**Patrón de arquitectura — Document Model vs Relational Model y Partition Strategy:**  
Por qué elegimos NoSQL para WealthLens y en qué casos habríamos elegido SQL. El modelo de documentos: cómo se mapean las entidades del dominio (Position, Property) a documentos JSON. Partition key design: la decisión de diseño más importante en Cosmos DB y cómo afecta al rendimiento y al coste. Repository Pattern: abstraer el acceso a datos para que la lógica de negocio no dependa de Cosmos DB.
...
**Patrón de arquitectura — Revisión C4 Model y Architecture Decision Records (ADR):**  
El modelo C4 de Simon Brown para documentar arquitectura en 4 niveles (Context, Containers, Components, Code). Diagramaremos WealthLens completa con este modelo. ADRs: cómo documentar las decisiones de arquitectura que tomamos durante el programa y por qué. Reflexión sobre qué patrones aplicamos, cuáles funcionaron y qué haríamos diferente en v2.

---

## 5. Mapa de patrones de arquitectura por módulo

| Módulo | Patrón principal | Concepto secundario |
|--------|-----------------|-------------------|
| M1 | Monorepo vs Polyrepo | Estructura de proyecto |
| M2 | Component-Based Architecture | Atomic Design, Separation of Concerns |
| M3 | Unidirectional Data Flow | State Management, Flux pattern |
| M4 | Infrastructure as Code | Immutable Deployments, Twelve-Factor App |
| M5 | Layered Architecture | REST constraints, API Gateway, Serverless |
| M6 | Delegation Pattern | Zero Trust, OAuth 2.0, Stateless Auth |
| M7 | Document Model | Partition Strategy, Repository Pattern |
| M8 | C4 Model | Architecture Decision Records |

---

## 6. Metodología de aprendizaje con Claude Code
...
- "¿Qué patrón de arquitectura estoy aplicando aquí y cuáles son sus trade-offs?"
...
## 9. Repositorio GitHub — estructura inicial
...
Cuando tengas esto listo, arrancamos con la explicación del primer patrón de arquitectura (Monorepo vs Polyrepo) y la creación del repositorio en GitHub.

---

*Documento generado en abril 2026 · WealthLens Learning Plan v2.0*  
*Actualización v2.0: incorporado hilo de formación en arquitectura de software a lo largo de todos los módulos*
